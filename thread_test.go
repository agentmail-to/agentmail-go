...
func TestThreadGetAttachment(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Threads.GetAttachment(
		context.TODO(),
		"attachment_id",
		agentmail.ThreadGetAttachmentParams{
			ThreadID: "thread_id",
		},
	)
	if err != nil {
...
func TestThreadSearchWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Threads.Search(context.TODO(), agentmail.ThreadSearchParams{
		Q:         "q",
		After:     agentmail.Time(time.Now()),
		Before:    agentmail.Time(time.Now()),
		Limit:     agentmail.Int(0),
