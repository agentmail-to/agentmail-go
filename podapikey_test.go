...
func TestPodAPIKeyNewWithOptionalParams(t *testing.T) {
...
func TestPodAPIKeyListWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestPodAPIKeyDelete(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	err := client.Pods.APIKeys.Delete(
		context.TODO(),
		"api_key_id",
		agentmail.PodAPIKeyDeleteParams{
			PodID: "pod_id",
		},
	)
	if err != nil {
